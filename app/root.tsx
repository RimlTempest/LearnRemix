...
/**
 * `link`エクスポートは，HTMLの`<link>`要素の属性に対応するオブジェクトの配列を返す関数です．
 * HTMLの `<link>` 要素の属性をマッピングするオブジェクトの配列を返す関数です。
 * これらは、アプリ内のすべてのルートに `<link>` タグをロードします。
 * 個々のルートは独自のリンクを含むことができます。
 * しかし、個々のルートには、ユーザーがルートから離れてナビゲートしたときに自動的にアンロードされる独自のリンクを含めることができます。
 *
...
/**
 * ルートモジュールのデフォルトのエクスポートは、現在のルートをレンダリングするコンポーネントです。
 * ルートをレンダリングするコンポーネントで、`<Outlet />` コンポーネントを介しています。
 * これは、アプリのグローバルなレイアウト * コンポーネントと考えてください。
 * コンポーネントと考えてください。
 */
...
/**
 * ルートが変更されたときに、スクリーンリーダーのユーザーにアラートを表示します。
 */
const RouteChangeAnnouncement = React.memo(() => {
  ...
  /*
   * サーバーでは何もレンダリングしません。ライブリージョンは、次のような場合を除き、何の価値も提供しません。
   * スクリプトが読み込まれ、ブラウザが通常のルーティングを引き継がない限り、ライブ領域は何の価値もありません。
   */
  if (!hydrated) {
    return null
  }
