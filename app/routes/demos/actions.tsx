...
// フォームがPOSTを送信すると、そのアクションはサーバー上で呼び出されます。
...
export let action: ActionFunction = async ({ request }) => {
  ...
  /*
   * 典型的なアクションワークフローは、ネットワーク経由で送られてきたフォームデータの検証から始まります。
   * 検証することから始まります。
   * クライアントサイドでの検証も良いですが、サーバサイドでの検証も必要です。
   * サーバーサイドにも必要です。
   * 問題がある場合は、データを返して、コンポーネントはそれをレンダリングすることができます。
   */
  ...
  /*
   * 最後に、データが有効であれば、通常、データベースへの書き込み、メールの送信、ユーザーのログインなどを行います。
   * 成功したアクションの後には、たとえ同じ場所であってもリダイレクトすることをお勧めします。
   * ブラウザからユーザーがクリックして戻ってきても、データが再投稿されないようにするためです。
   */
...
export default function ActionsDemo() {
  ...
  /*
   * このフォームはJavaScriptがなくても動作しますが、ある場合は、間違った答えの入力を選択することで、より良い体験をすることができます!
   * お使いのブラウザでJavaScriptを無効にして、何が起こるか見てみましょう。
   */
  useEffect(() => {
